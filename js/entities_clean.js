...
class Projectile {
    ...
    draw(ctx) {
        ...
        if (this.type === 'SPREAD' || this.type === 'HOMING' || this.type === 'ENEMY' || this.type === 'ENEMY_AIMED' || this.type === 'FUNNEL_SHOT') {
            ctx.beginPath();
...
class Enemy {
    ...
    update() {
        ...
        if (this.lives <= 0 && !this.markedForDeletion) {
            ...
            if (Math.random() < dr) {
                const types = ['RATE', 'SPREAD', 'LASER', 'HOMING', 'LIFE', 'BOMB', 'FUNNEL'];
...
class Player {
    constructor(game) {
        ...
        if (this.type === 'STORM') {
            this.speed *= 1.3;
            this.color = '#ff0';
            // Starts with 2 funnels
            this.addFunnel();
            this.addFunnel();
        } else if (this.type === 'BASTION') {
            this.speed *= 0.7;
            this.color = '#f00';
            this.shield = 2; // Energy Shield
            this.maxShield = 2;
        } else {
            // VANGUARD
            this.color = '#0ff';
    ...
    draw(ctx) {
        ...
        const colors = { DEFAULT: this.color, SPREAD: '#ff0', LASER: '#0ff', HOMING: '#f0f' };
        ctx.fillStyle = this.weaponType === 'DEFAULT' ? this.color : colors[this.weaponType];
        ...
        if (this.type === 'STORM') {
            ...
            ctx.fillRect(this.x + 20, this.y + 5, 10, 16);
